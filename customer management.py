...
def OnSelected(event):
    ...
    UpdateWindow.title("Customer Contact List")
    width = 400
    height = 300
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
Top.pack(side=TOP)
Mid = Frame(root, width=500,  bg="#4ff0e8")
Mid.pack(side=TOP)
MidLeft = Frame(Mid, width=100)
MidLeft.pack(side=LEFT, pady=10)
MidLeftPadding = Frame(Mid, width=370, bg="#4ff0e8")
...
lbl_title = Label(Top, text="Store's Customer Contact Management System", font=('arial', 16), width=500)
lbl_title.pack(fill=X)
